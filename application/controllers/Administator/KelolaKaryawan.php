...
class KelolaKaryawan extends CI_Controller {
    ...
    public function nonaktivasi($user_id) {
        ...
        // Lakukan nonaktivasi dengan menggunakan $keterangan yang didapatkan dari URL
    
        $this->KaryawanModel->nonaktivasi($user_id, $keterangan);
    ...
    public function simpanPerangkat() {
        ...
        if (!$this->upload->do_upload('foto')) {
            ...
            // Data perangkat
            $data_perangkat = array('nomer_seri' => $nomer_seri, 'nama_perangkat' => $nama_perangkat, 'kategori_id' => $kategori_id, 'spesifikasi' => $spesifikasi, 'departemen_id' => $lokasi_fisik, 'status_perangkat' => $status_perangkat, 'user_id' => $user_id, 'foto' => $foto, 'tanggal_masuk' => $tanggal_masuk);
            if (!empty($ipaddress)) {
    ...
    public function updatePerangkat($perangkat_id) {
        ...
        if ($_FILES['foto1']['name'] !== '') {
            ...
            if (!$this->upload->do_upload('foto1')) {
                ...
                $foto = $upload_data['file_name'];
                // Data perangkat
                $data_perangkat = array('nomer_seri' => $nomer_seri, 'nama_perangkat' => $nama_perangkat, 'kategori_id' => $kategori_id, 'spesifikasi' => $spesifikasi, 'status_perangkat' => $status_perangkat, 'catatan' => $catatan, 'user_id' => $user_id, 'foto' => $foto, 'tanggal_masuk' => $tanggal_masuk);
                if (!empty($ipaddress)) {
                    $data_perangkat['ipaddress'] = $ipaddress;
                }
